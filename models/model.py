...
class RainDropRemoval(object):
    ...
    def input_producer(self, batch_size=10):
        def read_data():
            ...
            img_a, img_b = tf.unstack(
                tf.random_crop(tf.stack([img_a, img_b], axis=0), [2, self.crop_size, self.crop_size, self.chns]),
                axis=0)
    ...
    def train(self):
        ...
        for step in range(sess.run(global_step), self.max_steps + 1):
            ...
            if step % 10 == 0:
                ...
                format_str = (
                    '%s: step %d, final_step %d, epoch %d, lr %.5f, loss = (%.5f; %.5f, %.5f)(%.1f data/s; %.3f s/bch)')
                print(format_str % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), step, self.max_steps,
                                    self.current_epoch, sess.run(self.lr), loss_total_val, 0.0, 0.0, examples_per_sec,
                                    sec_per_batch))
    ...
    def test(self, inputdata_path, output_path):
        if not os.path.exists(output_path):
            ...
            os.makedirs('./testing_result/attention_map')
        ...
        for i in range(num):
            # gt = imageio.imread(os.path.join(gt_path, gtsName[i]))
            rDrop = imageio.imread(os.path.join(input_path, imgsName[i]))
            h, w, c = rDrop.shape
            rDrop = rDrop[:, :, 0:3]
            print(rDrop.shape)
            ...
            #attention_map = sess.run(atte_map, feed_dict={inputs: rDropPad/255.0})
            duration = time.time() - start

            #attention_map = resize(attention_map[0,:,:,0], [2160, 3840], 'bicubic')
            ...
            imageio.imwrite(os.path.join(output_path, imgsName[i]), res)
            #imageio.imwrite(os.path.join("./testing_result/attention_map", imgsName[i]), attention_map)
